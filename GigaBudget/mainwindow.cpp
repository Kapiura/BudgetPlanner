...
MainWindow::MainWindow(QWidget *parent)
  ...
  ui->setupUi(this);
  dbHandler = new DatabaseManager("gigaBudget", "root", "!@#QWE123qwe",
                                  "localhost", 3306);
  up = new UserPanel();
  messDialog = new QMessageBox();

  dialogWindow = new QDialog();
  userDelete = new QDialogButtonBox(
      QDialogButtonBox::Ok | QDialogButtonBox::Cancel, dialogWindow);
...
void MainWindow::reloadInExSavGo() {
  ...
  query =
      QString(
          "SELECT amount, savings.currency, title, date, savings.description, "
...
void MainWindow::addingCategoriesItems() {
  ...
  queryString = QString("select title from goal where u_id=%1;")
...
void MainWindow::dailyQuote() {
  ...
  QString queryString =
      QString("SELECT * from quotes where id=%1").arg(currentDay);
...
void MainWindow::on_deleteButton_clicked() {
  ...
  if (dialogWindow->exec()) {
    ...
    if (dbHandler->deleteUser()) {
      qDebug()
          << QString("User %1 has been deleted").arg(DatabaseManager::userId);
      this->setDefaultPageIndex();
      up->deleteDynamicWidgets(ui->users);
      up->creatingLoginPanel(ui->users, dbHandler);
    } else
...
void MainWindow::on_userSettingsChange_clicked() {
  ...
  QString queryString =
      QString("Update users SET name='%1', description='%2' where idu = %3")
          .arg(newName)
          .arg(newDesc)
          .arg(DatabaseManager::getUserId());

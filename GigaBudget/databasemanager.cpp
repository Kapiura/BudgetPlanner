...
int DatabaseManager::getUserId() { return userId; }

QString DatabaseManager::getCurrentUsername() { return currentUsername; }

DatabaseManager::DatabaseManager(QObject *parent) : QObject{parent} {}

DatabaseManager::DatabaseManager(const QString &dbName, const QString &userName,
                                 const QString &password, const QString &host,
                                 const int &port)
    : _dbName(dbName), _userName(userName), _password(password), _host(host),
      _port(port) {
...
void DatabaseManager::addExpenses(int &id, double &amount, QString &currency,
...
void DatabaseManager::addIncomes(int &id, double &amount, QString &currency,
                                 QString &category, QString &desc) {
...
void DatabaseManager::addGoal(QString &title, double &amount, QString &currency,
                              QString &desc) {
  QString queryString =
      QString("insert into goal (title, goal_amount, current_amount, currency, "
...
void DatabaseManager::addSav(QString &title, double &amount, QString &currency,
                             QString &desc) {
  QString queryString =
      QString("SELECT idg FROM goal WHERE title = '%1'").arg(title);
  QSqlQuery query(queryString, db);
  if (query.next()) {
...
bool DatabaseManager::deleteUser() {
  QString queryString = QString("DELETE FROM users WHERE idu = '%1'")
                            .arg(DatabaseManager::userId);
  QSqlQuery query(db);
  query.prepare(queryString);
  if (query.exec()) {
...
bool DatabaseManager::deleteData(QString &table) {
  QString queryString = QString("delete from %1 where u_id = %2")
                            .arg(table)
